...
<div style="margin-top:20px;">
		<table class="table table-bordered">
			...
			<c:forEach items="${riskDTO}" var="dto">

				<tr>
					<td>${dto.risk_no}</td>
					<td>${dto.pro_name}</td>
					<td><a href='/risk/detail${pageMaker.makeSearch(pageMaker.cri.page)}
					&risk_no=${dto.risk_no}'>${dto.risk_title}</a></td>
					<td>${dto.mem_name}&nbsp;&nbsp;&nbsp;
					
					</td>
					<td><fmt:formatDate pattern="yyyy-MM-dd"	value="${dto.risk_date}" /></td>
					<td>
						<c:choose>
							<c:when test="${dto.risk_status == '0'}">
								<div class="btn btn-block btn-secondary btn-sm" style="display: inline; font-size: 20px;">
								처리중</div>
							</c:when>
							
							<c:when test="${dto.risk_status == '1'}">
...
<script>
	...
	$(document).ready(function() {
		...
		$('#newBtn').on("click", function(evt) {
			self.location = "/risk/regist?mem_id=${loginUser.mem_id}";
		});
	});
</script>
